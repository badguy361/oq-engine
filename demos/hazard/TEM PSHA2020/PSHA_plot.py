...
id = 117
folder = 'Yu2023'
df_site = pd.read_csv(f"{folder}({id})/hazard_curve-mean-PGA_{id}.csv",skiprows=[0])
df_gmf = pd.read_csv(f"{folder}({id})/hazard_map-mean_{id}.csv",skiprows=[0])
df_gmf = pd.read_csv(f"{folder}({id})/hazard_uhs-mean_{id}.csv",skiprows=[0])
x = df_site["lon"]
y = df_site["lat"]
